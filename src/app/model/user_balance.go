...
func (m *UserBalanceModel) GetByAddress(address string) (*UserBalance, error) {
	...
	err := db.DB.Scopes(NotDeleted).Where("address = ?", address).First(&balance).Error
	if err != nil {
		return nil, err
	}
	return &balance, nil
...
func (m *UserBalanceModel) Update(balance *UserBalance) error {
	return db.DB.Save(balance).Error
}

// DeleteByID 逻辑删除记录
func (m *UserBalanceModel) DeleteByID(id int64) error {
	return db.DB.Scopes(NotDeleted).Model(&UserBalance{}).
		Where("id = ?", id).
...
func (m *UserBalanceModel) Page(page, pageSize int) ([]*UserBalance, int64, error) {
	...
	res := db.DB.Scopes(NotDeleted).Model(&UserBalance{})
