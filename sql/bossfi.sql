...
create table bii_chain_index_status
(
    id           BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    chain_id     INTEGER        NOT NULL,
    last_block_num BIGINT NOT NULL,
    last_sync_time TIMESTAMP(6),
    create_time  TIMESTAMP(6),
    modify_time  TIMESTAMP(6)
);

comment on table bii_chain_index_status is '链最新区块高度表';
comment on column bii_chain_index_status.id is '主键ID';
comment on column bii_chain_index_status.chain_id is '链ID';
comment on column bii_chain_index_status.last_block_num is '最新区块高度';
comment on column bii_chain_index_status.last_sync_time is '最新同步时间';
comment on column bii_chain_index_status.create_time is '创建时间';
comment on column bii_chain_index_status.modify_time is '更新时间';
alter table bii_chain_index_status owner to bossfi;
create unique index idx_bii_chain_index_status_chain_id on bii_chain_index_status (chain_id);